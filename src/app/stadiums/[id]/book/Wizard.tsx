    ...
    const fetchBookedSlots = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setBookedSlots(data.bookings ? data.bookings.map((b: any) => b.startTime) : []);
            }
        } catch (e) {
            console.error(e);
        }
                ...
                {step === 3 && (
                    <section style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px' }}>
                            {[60, 90, 120].map(mins => (
                                <button key={mins} onClick={() => setDuration(mins)} style={{
                                    flex: '1 1 200px', padding: '24px 32px', borderRadius: '12px', display: 'flex', alignItems: 'center', gap: '12px',
                                    backgroundColor: duration === mins ? colors.primary : 'white',
                                    color: duration === mins ? 'white' : '#0f172a',
                                    border: duration === mins ? 'none' : `1px solid ${colors.primary}33`,
                                    boxShadow: duration === mins ? `0 10px 15px -3px ${colors.primary}33` : 'none',
                                    fontWeight: 'bold', fontSize: '1.125rem', cursor: 'pointer', transition: 'all 0.2s'
                                }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '32px' }}>
                            ...
                            <button onClick={() => setStep(4)} style={{ padding: '16px 48px', backgroundColor: colors.primary, color: 'white', borderRadius: '12px', fontWeight: 'bold', border: 'none', cursor: 'pointer' }}>المتابعة للدفع</button>
                        </div>
                    </section>
                )}
